...
_Doctor checks_ (`doctor.checksy.yaml`) — validate the dev environment (tools installed, env vars set, etc.). Run on container creation and with `just doctor`.
...
_Static checks_ (`static.checksy.yaml`) — validate the codebase (lint, format, etc.). Run in CI and with `just static`.
